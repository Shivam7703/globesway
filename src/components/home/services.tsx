...
export default function Services() {
  ...
  return (
    <section className="lg:px-28 md:px-20 sm:p-9 p-7 text-center relative slider1 ser-bg">
      {data?.title1 && (
      ...
      <Swiper
        {...swiperOptions}
        className={`mySwiper md:mt-16 md:mb-24 mt-9 w-full max-w-7xl px-5 ${uniqueId}`}
      >
        {data?.list.map((cards: any) => (
          <SwiperSlide key={cards.id} className="mb-14 w-full group  ">
            {/* Card Content */}
            ...
            <div className=" sticky -mt-32 w-4/5 mx-auto z-10">
              <div className="text-white text-4xl group-hover:bg-color2 duration-300 bg-color1 p-2 w-max">
                {cards?.icon}
              </div>
              <div className="bg-white w-full p-4">
                <Link href={cards?.href || "#"} className="">
                  ...
                  <p className="text-color1 font-bold text-sm opacity-0 -mt-4 mb-6 group-hover:opacity-100 group-hover:mt-2 group-hover:mb-0 transition-all duration-300">
                    Read More
                  </p>{" "}
                </Link>
              </div>
