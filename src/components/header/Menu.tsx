...
const Menu = ({ onTop, Isprimary, Items }: any) => {
  ...
  return (
    <ul
      className={`hidden items-center lg:gap-x-5 md:gap-x-0 font-medium w-max mx-auto md:flex`}
    >
      {Items.map((item: any) => (
        <li key={item?.id} className="group relative transition-all ">
          <Link
            ...
            className={`px-4 py-3 flex justify-center items-center gap-1 cursor-pointer ${
              item?.animate ? " animate-scal  text-zinc-900" : ""
            }
    ${
      activeItem === (item?.href?.split("/").filter(Boolean).pop() || "home") &&
      Isprimary
        ? "!text-color1"
        : Isprimary
        ? "text-zinc-700"
      ...
      {Isprimary && <Image src={logoall} className="max-w-72" alt="All Logo" />}{" "}
    </ul>
  );
};
