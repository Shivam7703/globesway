      ...
      <div className="flex relative gap-y-6 z-10  justify-between items-stretch flex-wrap">
        {data &&
          data.map((card: any) => (
            <div
              key={card.id}
              className="p-3 md:w-[31.6%] sm:w-[48.5%] w-full sm:p-5 rounded-lg border-2 shadow-lg relative"
            >
              <Image
                title="Best immigration consultants in Delhi"
                src={missionbg}
                alt={"Best immigration consultants in Delhi"}
                className="absolute z-0 bottom-0 w-full h-auto object-cover left-0 opacity-100"
              />
              <div className="flex justify-center rounded-full items-center mx-auto sm:w-24 shadow-md sm:h-24 bg-gradient-to-l from-blue-900 to-color2 w-14 h-14 sticky sm:-mt-16 -mt-7 text-white  text-xl sm:!text-3xl">
                {card.icon}
              </div>
              <h2 className="text-xl my-3 leading-[1.9] text-center sm:text-2xl text-black">
                {card?.title}
              </h2>
              <div className="border-b border-zinc-800 max-w-44 mx-auto"></div>
              <p className="text-center text-zinc-600 my-3">{card?.para}</p>
            </div>
          ))}
      </div>
    </section>
